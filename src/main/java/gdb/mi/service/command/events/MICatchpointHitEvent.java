...
public class MICatchpointHitEvent extends MIBreakpointHitEvent {
	...
	/** @since 5.0 */
	protected MICatchpointHitEvent(int token, MIResult[] results, MIFrame frame, String bkptno,
			String reason) {
		super(token, results, frame, bkptno);
		fReason = reason;
	}
	...
	public static MIBreakpointHitEvent parse(int token, MIResult[] results,
		...
		try {
			...
			return new MICatchpointHitEvent(token, results, stoppedEvent.getFrame(),
	 ...
	 * {@link MIBreakpointHitEvent#parse(int, MIResult[])
	 ...
	 * @since 5.0
	 */
	public static MICatchpointHitEvent parse(int token, MIResult[] results, String bkptNumber,
			String gdbKeyword) {
		MIStoppedEvent stoppedEvent = MIStoppedEvent.parse(token, results);
		return new MICatchpointHitEvent(token, results, stoppedEvent.getFrame(),
